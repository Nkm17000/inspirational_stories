...
def get_cta_latin_font(size, bold=True):
    """
    Return a reliable Latin font for the English CTA card.

    The CTA/end card is English-only, so we deliberately use
    a Latin font instead of the Hindi/Devanagari subtitle font.
    """
...
def create_end_card(duration=END_CARD_DURATION):
    ...
    The HTML page cannot be embedded as a live clickable HTML element
    inside an MP4. This function recreates the CTA design as a video
    card and adds a scannable QR code.
    """
    ...
    # Simple layered background gradient.
    ...
    title_font = get_cta_latin_font(
        42,
        bold=True
    )

    tagline_font = get_cta_latin_font(
        24,
        bold=False
    )

    message_font = get_cta_latin_font(
        25,
        bold=True
    )

    button_font = get_cta_latin_font(
        24,
        bold=True
    )

    small_font = get_cta_latin_font(
        18,
        bold=False
    )

    footer_font = get_cta_latin_font(
        19,
        bold=True
    )
